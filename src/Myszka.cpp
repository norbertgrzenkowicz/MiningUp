...
void Myszka::positioning(sf::RenderWindow& window,
                         sf::Vector2f gamerPos,
                         sf::FloatRect gamerBounds)   // Kompletna metoda pozycjonowania myszki oraz kafli wzgledem zasiegu postaci
{
    ...
    // Koordynaty myszki poza zasi�giem postaci

    // tangensy dla trojk�ta tworzonego przez �rodek postaci pozycji myszki oraz przeci�cia prostych X oraz Y
    ...
    if (mousePosView.x >= gridSizef * 6 && mousePosView.x <= gridSizef * 24 && mousePosView.x <= gamerPosMid.x + destroyRange && mousePosView.y <= gamerPosMid.y + destroyRange
        && mousePosView.x >= gamerPosMid.x - destroyRange && mousePosView.y >= gamerPosMid.y - destroyRange)
    {
        tileSelector->setPosition(mousePosGrid.x * gridSizef, mousePosGrid.y * gridSizef);
             ...
             && (mousePosView.x > gamerPosMid.x + destroyRange || mousePosView.x < gamerPosMid.x - destroyRange || mousePosView.y > gamerPosMid.y + destroyRange
                 || mousePosView.y < gamerPosMid.y - destroyRange))
    {
        tileSelector->setPosition(rangeLimitGridPos.x * gridSizef, rangeLimitGridPos.y * gridSizef);
    }

    // Pozycjonowanie pomocniczych punkt�w okreslajacych zasieg oraz jego po�ow�
...
void Myszka::draw(sf::RenderWindow& window)   // rysowanie wszystkich elementow
{
    ...
    // odkomentuj dla dodatkowego interfejsu wskazuj�cego maksymalny dystans oraz jego po�ow�
    // window.draw(testBox);
