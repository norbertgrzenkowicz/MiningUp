...
void TileMap::destroyTile(const float& dt, sf::Event& event)
{
    ...
    if (sf::Mouse::isButtonPressed(sf::Mouse::Left) && tileSelectorPos.x >= 6 && tileSelectorPos.x < 36 && typ_kafli[tileSelectorPos.y][tileSelectorPos.x - 6] != 6)
    {
...
sf::Vector2f
TileMap::tilesDown(sf::Vector2f playerPos)   // Jesli obecna pozycja zostanie zmniejszona wzgledem osi X, dodatkowy rzad kaflow powstanie, a ostatni rzad zostanie usuniety
{
