...
private:
	...
	//Tekstura liczników surowców
	sf::Texture countUIT;
	sf::Sprite countUI;

	//Kontener przycisków dla menu pauzy
...
public:
	...
	bool endGameMenu(bool isDead, sf::Event& event, sf::RenderWindow& window);												//Menu poœmiertne
	void playersScore(sf::Event& event, sf::RenderWindow& window);															//Menu statystyk graczy
	void chooseDifficulty(sf::Event& event, sf::RenderWindow& window);														//Menu wyboru poziomu trudnoœci
	...
	void saveDead(unsigned int diamondsCount, unsigned int ironCount, unsigned int difficulty);								//Zapis statystyk po œmierci
	...
	void set_escape(bool esc);																								//Ustawianie booleanu zapytania wyjœcia z gry
	void set_renew(bool renewG);																							//Ustawianie booleanu odnowienia gry
	bool get_renew();																										//Odczyt booleanu odnowienia gry
	void set_difficulty(unsigned int difficulty);																			//Ustawianie poziomu trudnoœci gry
	unsigned int get_difficulty();																							//Odczyt poziomu trudnoœci gry
	...
	unsigned int get_loadeddiamonds();																						//Odczyt zebranych diamentów z pliku
	unsigned int get_loadediron();																							//Odczyt zebranego ¿elaza z pliku
	unsigned int get_loadedDifficulty();																					//Odczyt poziomu trudnoœci z pliku
};